...
export const CreditCardWidget = () => {
  ...
  if (loading) return <div>Loading credit info...</div>;
    ...
    <Card className="bg-gradient-to-br from-primary/10 to-secondary/10 border-primary/20">
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <div className="space-y-1">
            ...
            <div className="flex justify-between text-xs">
              <span>Used: SAR {creditAccount.credit_utilization.toLocaleString()}</span>
              <span className={isNearLimit ? 'text-red-600 font-semibold' : 'text-green-600'}>
                Available: SAR {creditAccount.available_credit.toLocaleString()}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-semibold">{creditAccount.payment_period_days} days</span>
        </div>
        ...
        {isNearLimit && (
          <div className="flex items-center gap-2 text-sm text-orange-600 bg-orange-50 p-2 rounded">
            ...
            <span>Approaching credit limit ({utilizationPercentage.toFixed(1)}%)</span>
          </div>
        )}
        ...
        {hasOverdue && (
          <div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 p-2 rounded">
            <AlertTriangle className="w-4 h-4" />
            <span>
              {creditAccount.days_overdue} days overdue | SAR {creditAccount.overdue_amount.toLocaleString()}
            </span>
          </div>
        )}
        ...
        {isOnHold && (
          <div className="flex items-center gap-2 text-sm text-red-700 bg-red-100 p-2 rounded font-semibold">
            <AlertTriangle className="w-4 h-4" />
            <span>Account on hold - contact support to resolve</span>
          </div>
        )}
      </CardContent>
    </Card>
