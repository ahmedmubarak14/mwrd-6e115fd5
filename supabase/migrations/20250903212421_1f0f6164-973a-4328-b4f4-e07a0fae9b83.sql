-- Secure remaining vendor tables with critical business data

-- 1. Fix vendor_public_info if still accessible to public
-- Ensure RLS is enabled and no public policies exist
DROP POLICY IF EXISTS "Enable read access for all users" ON vendor_public_info;
DROP POLICY IF EXISTS "Public read access" ON vendor_public_info;
DROP POLICY IF EXISTS "Anyone can view approved vendors" ON vendor_public_info;

-- 2. Secure vendor_categories table
...
-- 3. Secure vendor_profiles_extended table  
