...
-- Fix: allow updated_at to be set by its own trigger without failing validation.
-- We keep all intended protections but stop checking created_at/updated_at.
...
BEGIN
  ...
  -- Suppliers: can update their own offer, but NOT client approval fields
  ...
  -- Request owner: only approval fields can change
  IF auth.uid() = req_owner THEN
    IF COALESCE(NEW.title, OLD.title) IS DISTINCT FROM COALESCE(OLD.title, NEW.title) OR
       ...
       COALESCE(NEW.supplier_id::text, OLD.supplier_id::text) IS DISTINCT FROM COALESCE(OLD.supplier_id::text, NEW.supplier_id::text) THEN
